...
with st.sidebar:
    st.header('输入文件')
    file_compare = st.file_uploader('11月比较.xlsx', type=['xlsx'])
    file_rawlist = st.file_uploader('原料清单.xlsx', type=['xlsx'])
    file_q3 = st.file_uploader('蚌埠二厂系统成本-Q3.xlsx', type=['xlsx'])
...
if file_compare and file_rawlist and file_q3:
    ...
    try:
        ...
        display_legs = legs_df.drop(columns=[c for c in HIDE_COLS if c in legs_df.columns])
        display_breast = breast_df.drop(columns=[c for c in HIDE_COLS if c in breast_df.columns])
        display_other = other_df.drop(columns=[c for c in HIDE_COLS if c in other_df.columns])
        display_legs = display_legs.drop(columns=['行序'], errors='ignore')
        display_breast = display_breast.drop(columns=['行序'], errors='ignore')
        display_other = display_other.drop(columns=['行序'], errors='ignore')
        fmt = {
            '修形前原料综合耗用单价': '{:.2f}',
            '半成品原料成本': '{:.2f}',
            '半成品修形人工成本': '{:.2f}',
        ...
        prefix = file_compare.name.split('_')[0] if '_' in file_compare.name else file_compare.name.split('.')[0]
        data = to_excel_bytes(
            display_legs,
            display_breast,
            bb2_all,
        ...
        if (not display_legs.empty) or (not display_breast.empty) or (not display_other.empty):
            st.session_state['download_data'] = data
        st.success('计算完成')
        st.subheader('腿肉')
        st.dataframe(display_legs.style.format(fmt), use_container_width=True)
        st.subheader('胸肉')
        st.dataframe(display_breast.style.format(fmt), use_container_width=True)
        st.subheader('其他')
        st.dataframe(display_other.style.format(fmt), use_container_width=True)
    except Exception as exc:
        st.error(f'计算失败: {exc}')
        try:
            preview = _load_compare_df(file_compare, sheet_name=0)
            st.caption('当前识别到的列名:')
